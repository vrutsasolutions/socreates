  ...
  return (
    ...
    >
      <Avatar
        ...
        seed={person.id || person.userId}
      />
      <div className="flex-1 min-w-0">
        <div className="text-[#0D2137] font-semibold text-[15px] truncate">
          {person.name || "User"}
        </div>
...
export default function FollowList() {
  ...
  const initialTab =
    params.get("tab") === "following" ? "following" : "followers";
  ...
  const handleUnfollow = (person) => {
    ...
    unfollowUser(person.userId).catch(() =>
      setFollowing((prev) => [person, ...prev]),
    );
  };
    ...
    <div className="min-h-screen bg-[#F4F7FF] flex flex-col">
      ...
      <div className="bg-[#1565C0]">
        <div className="bg-white rounded-t-[32px] pt-5 pb-10 flex flex-col min-h-screen">
            ...
            {loading ? (
              Array(4)
                ...
                .map((_, i) => (
                  <div
                    key={i}
                    className="bg-[#F0F6FF] border border-[#BBDEFB] rounded-2xl p-3.5 flex items-center gap-3 animate-pulse"
                  >
              ...
              <div className="text-center py-14">
                <div className="w-14 h-14 rounded-2xl bg-[#F0F6FF] border border-[#BBDEFB] flex items-center justify-center mx-auto mb-3">
                  ...
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.768-.231-1.48-.631-2.066M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.768.231-1.48.631-2.066M15 7a3 3 0 11-6 0 3 3 0 016 0z"
                    />
                  </svg>
                </div>
                ...
                <p className="text-[#90A4AE] text-xs mt-1">
                  {tab === "followers"
                    ? "Share your ideas to grow your audience"
                    : "Explore creators to follow"}
                </p>
              </div>
            )}
          </div>
          ...
          {/* Suggested for you */}
          {tab === "following" && !loading && suggested.length > 0 && (
            <div className="mt-8 px-4">
              <div className="border-t border-[#EAF2FF] pt-5">
                <h2 className="text-[#1565C0] font-bold text-[15px] mb-1.5">
                  Suggested for you
                </h2>
                <div>
                  {suggested.map((c) => (
                    <PersonRow
                      ...
                      person={{
                        ...c,
                        profileImage: c.profileImage || c.avatarUrl,
                      }}
                      action={
                        <FollowBtn
