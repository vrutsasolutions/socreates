...
/**
 * Full-page payout setup form — first-time only. Once a creator has
 * details saved, changing them goes through the lighter dedicated
 * UpdateBankAccount.jsx flow (Figma screen 4) instead of this page, so
 * this form no longer has a "change" mode.
 *
...
export default function PayoutSetup() {
  ...
  const validate = () => {
    ...
    if (!/^[6-9]\d{9}$/.test(form.mobile.trim())) return 'Enter a valid 10-digit mobile number.';
    if (!/^[A-Z]{5}[0-9]{4}[A-Z]$/.test(form.pan.trim().toUpperCase())) return 'Enter a valid PAN (e.g. ABCDE1234F).';

    if (method === 'vpa') {
      if (!/^[\w.-]+@[\w.-]+$/.test(form.vpa.trim())) return 'Enter a valid UPI ID (e.g. name@bank).';
    } else {
    ...
    if (!form.confirmOwnership) return 'Please confirm this account belongs to you.';
    return '';
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(errMsg(e, 'Could not save payout details. Please check your details and try again.'));
    } finally {
      setBusy(false);
    }
  };
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 lg:w-12 lg:h-12 rounded-full bg-white/20 flex items-center justify-center shrink-0">
                <svg className="w-5 h-5 lg:w-6 lg:h-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                </svg>
              </div>
              <div className="min-w-0">
                <h1 className="text-white font-bold text-lg lg:text-2xl">Complete payout setup</h1>
        ...
        <div className="flex-1 overflow-y-auto px-5 sm:px-8 lg:px-10 pt-6 pb-8">
          <div className="max-w-2xl mx-auto">
            {loading ? (
              ...
              {/* ── Personal details ──────────────────────────────── */}
              <div>
                <h2 className="text-[#0D2137] text-sm lg:text-base font-bold mb-3">Personal details</h2>
                <div className="space-y-3.5">
                  <Field label="Full legal name *">
                    <input value={form.legalName} onChange={set('legalName')}
                      placeholder="As per your government ID" className={inputCls} autoFocus />
                  </Field>
                  <Field label="Email *">
                  ...
                  <Field label="Mobile number *">
                    <input value={form.mobile} onChange={set('mobile')} inputMode="numeric"
                      maxLength={10} placeholder="10-digit mobile number" className={inputCls} />
                  </Field>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <h2 className="text-[#0D2137] text-sm lg:text-base font-bold">Bank details</h2>
                  <div className="flex gap-1 bg-[#F0F6FF] p-0.5 rounded-lg">
                    <MethodTab active={method === 'bank_account'} onClick={() => setMethod('bank_account')} label="Bank" />
                    <MethodTab active={method === 'vpa'} onClick={() => setMethod('vpa')} label="UPI" />
                  </div>
                </div>

                {method === 'vpa' ? (
                  <Field label="UPI ID *">
                    <input value={form.vpa} onChange={set('vpa')} placeholder="name@bank" className={inputCls} />
                  </Field>
                ) : (
                  <div className="space-y-3.5">
                    <Field label="Account holder name *">
                      <input value={form.legalName} disabled placeholder="As it appears on your passbook"
                        className={`${inputCls} opacity-60 cursor-not-allowed`} />
                    </Field>
                    <Field label="Bank account number *">
                      <input value={form.accountNumber} onChange={set('accountNumber')} inputMode="numeric"
                        placeholder="Enter your account number" className={inputCls} />
                    </Field>
                    <Field label="Confirm account number *">
                      <input value={form.confirmAccountNumber} onChange={set('confirmAccountNumber')} inputMode="numeric"
                        placeholder="Re-enter your account number" className={inputCls} />
                    </Field>
                    ...
                    <Field label="Bank name">
                      <div className="relative">
                        <input value={form.bankName} onChange={set('bankName')} placeholder="Auto-fills from IFSC"
              ...
              {/* ── Tax details ────────────────────────────────────── */}
              <div>
                <h2 className="text-[#0D2137] text-sm lg:text-base font-bold mb-3">Tax details</h2>
                <div className="space-y-3.5">
                  <Field label="PAN number * (Individual Identification)">
                    <input value={form.pan} onChange={set('pan')} placeholder="e.g. ABCDE1234F"
                      maxLength={10} className={`${inputCls} uppercase`} />
                  </Field>

                  <label className="flex items-start gap-2.5 pt-1 cursor-pointer">
                    <input type="checkbox" checked={form.confirmOwnership} onChange={set('confirmOwnership')}
