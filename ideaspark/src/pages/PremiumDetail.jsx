...
export default function PremiumDetail() {
  ...
  return (
    <div className="min-h-screen bg-[#F0F6FF]">
      ...
      {/* Header */}
      <div className="sticky top-0 z-30 bg-[#1565C0] px-4 py-4 flex items-center gap-3">
        <button
          onClick={() => navigate(-1)}
          className="w-9 h-9 flex items-center justify-center text-white transition">
      ...
      <div className="px-4 py-5 max-w-lg mx-auto">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <div className="text-black font-semibold text-sm">{idea?.creatorName}</div>
            <div className="text-[#90A4AE] text-xs">{idea?.category}</div>
          </div>
        </div>

        {/* Title */}
        <h1 className="text-black text-2xl font-bold leading-tight mb-4">{idea?.title}</h1>
        ...
        {isLocked ? (
          <div>
            ...
            <div className="mt-6 bg-white border border-[#BBDEFB] rounded-3xl p-6 text-center shadow-sm">
              <div className="text-4xl mb-3">🔐</div>
              <h3 className="text-black font-bold text-lg mb-2">Unlock Premium Ideas</h3>
              <p className="text-[#546E7A] text-sm mb-5">
                Get unlimited access to all premium ideas.
              </p>
              <button
                onClick={() => navigate('/membership')}
                className="w-full bg-[#1565C0] hover:bg-[#0D47A1] text-white font-bold py-3.5 rounded-2xl active:scale-95 transition-all shadow-md shadow-blue-200">
                Upgrade to Premium →
              </button>
            </div>
          </div>
