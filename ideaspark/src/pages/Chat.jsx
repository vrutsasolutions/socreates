...
function Bubble({ m, onImageClick, onReply }) {
  ...
  if (m.type === "image") {
    ...
    content = (
      <div className="block">
        ...
        {m.isVideo ? (
          ...
          <button onClick={() => onImageClick(src)} className="block">
            {media}
          </button>
        )}
        {m.text && (
    ...
    content = (
      <div
        className={`flex items-center gap-3 rounded-[18px] px-4 py-3 max-w-[240px] ${mine ? "bg-[#1565C0] text-white" : "bg-white text-[#0D2137] shadow-sm"}`}
      >
        <span
  ...
  return (
    <div className={`flex flex-col ${mine ? "items-end" : "items-start"}`}>
      ...
      {m.time && (
        <span
          className={`text-[10px] text-[#90A4AE] mt-1 px-2 ${mine ? "text-right" : "text-left"}`}
        >
          {m.time}
        </span>
      )}
