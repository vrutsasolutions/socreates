...
public class CreatorService {
        ...
        // Formula: raw_score = (views*0.25) + (saves*0.40) + (likes*0.20) +
        // (comments*0.15)
        // Stored as scaled longs (*100) to avoid floating-point drift:
        // views*25 + saves*40 + likes*20 + comments*15 (divide by 100 for the true
        // decimal score)
        ...
        public List<CreatorEarningDTO> getEarnings(String email) {
                ...
                List<CreatorEarning> rows = earningRepository.findByCreatorIdOrderByMonthDesc(creator.getId());

                return rows.stream()
                                .map(row -> {
                                        // score_percent stored in DB; convert to 0-100 int for display
                                        int scoreDisplay = row.getScorePercent() != null
                                                        ? row.getScorePercent().setScale(0, RoundingMode.HALF_UP)
                                                                        .intValue()
                                                        : 0;
                                        ...
                                        return CreatorEarningDTO.builder()
                                                        ...
                                                        .scheduledFor(row.getScheduledFor() != null
                                                                        ? row.getScheduledFor().format(DATETIME_FMT)
                                                                        : null)
                                                        .paidAt(row.getPaidAt() != null
                                                                        ? row.getPaidAt().format(DATETIME_FMT)
                                                                        : null)
                                                        .destination(maskDestination(row.getPayoutAccount()))
                                                        .failureReason(row.getFailureReason())
                                                        .retryCount(row.getRetryCount() != null ? row.getRetryCount()
                                                                        : 0)
                                                        .build();
                                })
                                .toList();
        ...
        protected void upsertCurrentMonthEarning(User creator, LocalDate month) {
                ...
                CreatorEarning earning = earningRepository
                                ...
                                .orElseGet(() -> CreatorEarning.builder()
                                                ...
                                                .month(month)
                                                .status(
                                                                creator.getActivePayoutAccount() == null
                                                                                ? "Setup_Missing"
                                                                                : "Scheduled")
                                                .build());
                ...
                // Automatically recover once the creator completes payout setup.
                if ("Setup_Missing".equals(earning.getStatus())
                                && creator.getActivePayoutAccount() != null) {
                ...
                return earningRepository
                                ...
                                poolRepository.findByMonth(month)
                                                ...
                                                .map(pool -> metrics.getSharePercent()
                                                                ...
                                                                .multiply(BigDecimal
                                                                                .valueOf(pool.getCreatorPoolPaise()))
                                                                .longValue())
                                                .orElse(0L));
        }
         ...
         * the month (see
         * CreatorMonthlyMetricsRepository#sumRawScoreForEligibleCreators).
         *
        ...
        private static String maskDestination(PayoutAccount account) {
                ...
                if (acct != null && !acct.isBlank()) {
                        ...
                        String bank = account.getPayoutIfsc() != null && !account.getPayoutIfsc().isBlank()
                                        ? account.getPayoutIfsc().substring(0, 4)
                                        : "BANK";
                        return bank + " ****" + last4;
                }
                return null;
